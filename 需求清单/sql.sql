...
(
    ...
    recipient json not null comment '接受人',
    ...
    email varchar(255) null comment '接受者(供索引使用)'
)
...
(
    ...
    description text not null comment '详细信息',
    submitter_id tinyint null comment '发送者',
    created_at timestamp default CURRENT_TIMESTAMP not null,
    updated_at timestamp default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP,
    deleted_at timestamp null,
    read_at timestamp null comment '阅读时间',
    module json null comment '关于当前提醒包含的一些数据'
)
...
(
    id bigint unsigned auto_increment
        primary key,
    name varchar(60) default '' not create table admins
        (
        id int auto_increment comment '主键ID'
        primary key,
        account varchar(60) null comment '用户名称',
